<template>
    <div>
        <div>
            <el-button size="small" type="primary" @click="handleQuery"v-auth code="account:conclude:list">刷新列表</el-button>
            <el-button size="small" type="primary" @click="showApplicationDetails"v-auth code="account:conclude:detail">查看申请</el-button>
            <el-button size="small" type="primary" @click="showProject" v-auth code="account:project:detail">查看项目</el-button>
            <el-button size="small" type="primary" @click="applyPass" v-auth code="account:conclude:pass">申请通过</el-button>
            <el-button size="small" type="danger" @click="applyNotPass" v-auth code="account:conclude:notpass">申请驳回</el-button>
        </div>
        <br/>
        <el-table
            :data="tableData"
        ...
        <br/>
        <el-dialog title="结题驳回" :visible.sync="updateApplicationNotPassVisible" size="small" >
            <el-form :model="form" >
        ...
        <el-dialog title="结题详情" :visible.sync="detailDialogVisible" size="small" >
            <el-form :model="form" >
                ...
                <el-form-item label="结题说明" :error="errors.name">
                    <el-input type="textarea"  v-model="form.projectCompletion"  readonly="readonly" ></el-input>
                </el-form-item>
                <el-form-item label="驳回理由" :error="errors.name">
...
<script>
    ...
    export default {
        ...
        methods:{
            handleQuery() {
                this.$axios.get(this.$config.SYSTEM_HOST + "/conclude_application/query").then((res) => {
            ...
            applyPass(){
                if(this.currentRow.status == '申报中') {
                    this.$axios.put(this.$config.SYSTEM_HOST + "/conclude_application/pass", this.currentRow).then((res) => {
                        this.handleQuery();
                    });
                }else {
            ...
            handleNotPass(){
                this.$axios.put(this.$config.SYSTEM_HOST + "/conclude_application/not_pass", this.form).then((res) => {
            ...
            showApplicationDetails(){
                this.$objectUtils.clear(this.errors);
                this.$objectUtils.clear(this.form);
                this.$axios.get(this.$config.SYSTEM_HOST + "/conclude_application?id=" + this.currentRow.id).then((res) => {
